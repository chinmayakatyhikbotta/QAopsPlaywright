...
const {
  getEventHubBaseUrl,
  getEventHubCredentials,
  hasEventHubCredentials,
} = require('../utils/env');
...
async function loginAndGoToEvents(page, baseUrl) {
  const { email, password } = getEventHubCredentials();
...
test.describe('@API EventHub network mocking', () => {
  test.skip(
    !hasEventHubCredentials(),
    'Set EVENTHUB_EMAIL and EVENTHUB_PASSWORD in .env'
  );

  test('@API banner is visible when 6 events are returned', async ({ page }) => {
    const baseUrl = getEventHubBaseUrl();
    await loginAndGoToEvents(page, baseUrl);
  ...
  test('@API banner is hidden when fewer than 6 events are returned', async ({
    page,
  }) => {
    const baseUrl = getEventHubBaseUrl();
    await loginAndGoToEvents(page, baseUrl);
